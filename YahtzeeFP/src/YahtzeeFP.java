...
// The Final Project will build on Project 3, however the Final Project DOES NOT
//    rely on a working Project 1, 2, or 3. You do not need to insert any code
//    from Projects 1, 2, or 3 in this Project.
//
// The goal will be to rewrite/replace code to utilize OOP (Object-Oriented Programming).
//
...
class Yahtzee extends Game {
    ...
    // overloaded public void method created for displayScoreSheet method
    // this method calls displayScoreSheet with argument for System.out
    public void displayScoreSheet() {
        this.displayScoreSheet(System.out);
    }

    // This method displays the current scorecard.
    // modified method by adding one parameter named outStream of datatype PrintStream
    public void displayScoreSheet(PrintStream outStream) {
        ...
        // replaced System.out with outStream
        outStream.println();
...
public class YahtzeeFP {
    ...
    public static void main(String[] args) {
        ...
        // declared new variable of Yahtzee datatype and assigned it to a new instance of the Yahtzee class
        //  called GameOfYahtzee
        ...
        do {
            ...
            // invoked the getPressEnterMessage method on variable named GameOfYahtzee and passed no arguments in
            // the return value of that method call is printed out using the System.out.println method

            System.out.print(GameOfYahtzee.getPressEnterMessage());

            input.nextLine();

            // invoked the setTurnCount setter on variable named GameOfYahtzee and passed no arguments in
            // the return value of that method call is printed out using the System.out.println method
            GameOfYahtzee.setTurnCount(GameOfYahtzee.getTurnCount());

            // Roll the dice.
            for (int i = 0; i < Yahtzee.NUMBER_OF_DICE; i++) {

                // invoked the getRandomInt method on variable GameOfYahtzee and passed it, along with the for-loop's
                //  control variable, into a method call of setDice
                GameOfYahtzee.setDice(i, GameOfYahtzee.getRandomInt());

            }

            // invoked method setNumberOfRolls on variable GameOfYahtzee and passed in the argument of 1
            GameOfYahtzee.setNumberOfRolls(1);
            // invoked method displayDice on variable GameOfYahtzee and passed in no variables
            GameOfYahtzee.displayDice();
            // invoked setter method of setTurnOver on variable GameOfYahtzee and passed in false for argument
            ...
            do {
                ...
                switch (dice2Reroll.toUpperCase()) {
                    ...
                    // replaced existing code with CONSTANT call to Yahtzee class method
                    // case's code-block contains setter method calls to GameOfYahtzee variable and passing in true argument
                    case Yahtzee.EXIT_RESPONSE:
                        GameOfYahtzee.setTurnOver(true);
                        GameOfYahtzee.setGameExit(true);
                        break;
                    // replaced existing code with CONSTANT call to Yahtzee class method
                    // case's code-block contains a method call of displayScoreSheet to variable GameOfYahtzee
                    case Yahtzee.SCORE_CARD_RESPONSE:
                        GameOfYahtzee.displayScoreSheet();
                        break;
                    // replaced existing code with CONSTANT call to Yahtzee class method
                    // case's code-block contains a method call of displayDice to variable GameOfYahtzee
                    case Yahtzee.DISPLAY_DICE_RESPONSE:
                        GameOfYahtzee.displayDice();
                        break;
                    // replaced existing code with CONSTANT call to Yahtzee class method
                    // case's code-block contains a method call of setTurnOver setter method to variable GameOfYahtzee
                    case Yahtzee.END_TURN_RESPONSE:
                        GameOfYahtzee.setTurnOver(true);
                        break;
                    // case's code-block contains a method call of displayErrorMessage to variable GameOfYahtzee
                    case "":
                    ...
                    default:
                        ...
                        if (checkDice2Reroll.isBlank()) {
                            ...
                            // call to getNumberOfRolls getter method to GameOfYahtzee variable and passed it as the
                            //  argument to setter method call of setNumberOfRolls to GameOfYahtzee variable call
                            GameOfYahtzee.setNumberOfRolls(GameOfYahtzee.getNumberOfRolls());
                            GameOfYahtzee.displayDice();

                            // if-statement to check for equality between getNumberOfRolls getter method call to GameOfYahtzee
                            //  variable against getMaxNumberRolls getter method call to GameOfYahtzee variable
                            // if the if-statement's condition is true, then call the setTurnOver method and pass in true
            ...
            if (!GameOfYahtzee.isGameExit()) {
                ...
                // invoke method of displayScoreSheet on variable GameOfYahtzee and pass in no arguments
                GameOfYahtzee.displayScoreSheet();
                ...
                do {
                    ...
                    switch (scoreOptionInput.toUpperCase()) {
                        ...
                        // replaced existing code by invoking CONSTANT from Yahtzee class
                        // code block calls setter method of setGameExit against GameOfYahtzee variable and pass in true
                        case Yahtzee.EXIT_RESPONSE:
                            GameOfYahtzee.setGameExit(true);
                            break;
                        // replaced existing code by invoking CONSTANT from Yahtzee class
                        // code block calls displayScoreSheet method against GameOfYahtzee variable
                        case Yahtzee.SCORE_CARD_RESPONSE:
                            GameOfYahtzee.displayScoreSheet();
                            break;
                        // replaced existing code by invoking CONSTANT from Yahtzee class
                        // code block calls displayDice method against GameOfYahtzee variable
                        case Yahtzee.DISPLAY_DICE_RESPONSE:
                            GameOfYahtzee.displayDice();
                            break;
                        // code block for case-statement invokes the displayErrorMessage against variable GameOfYahtzee
                        case "":
                            GameOfYahtzee.displayErrorMessage();
                            break;
        ...
        // call to method displayScoreSheet against GameOfYahtzee method
        GameOfYahtzee.displayScoreSheet();

        // declared new variable called outputFile of File datatype
        // created a new instance of File class and passed in CONSTANT variable of OUTPUT_FILE_NAME
        // assigned new instance call to newly declared variable
        File outputFile = new File(OUTPUT_FILE_NAME);

        // created new try-catch block
        // declared new variable named outputStream of PrintStream datatype
        // call to displayScoreSheet method with outputStream as argument
        // catch block prints out the error message CONSTANT
        try {
